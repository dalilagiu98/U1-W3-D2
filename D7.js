...
const evenNumber = (number) => {
  const even = arrayNumber.filter((number) => number % 2 === 0);
  return even;
};
...
const sumReduce = (array) => {
  const sum = arrayNumber.reduce(function (accumulator, number) {
    return accumulator + number;
  }, 0);
  return sum;
};

console.log(sumReduce(arrayNumber));

/* ESERCIZIO 6 (map)
  Scrivi una funzione che, dato un array di soli numeri e un numero n come parametri, ritorni un secondo array con tutti i valori del precedente incrementati di n
*/

const newFunction = (array, n) => {
  const newArray = array.map((number) => number + n);
  return newArray;
};

console.log(newFunction(arrayNumber, 2));

/* ESERCIZIO 7 (map)
  Scrivi una funzione che, dato un array di stringhe, ritorni un nuovo array contenente le lunghezze delle rispettive stringhe dell'array di partenza
  es.: ["EPICODE", "is", "great"] => [7, 2, 5]
*/
const arrayString = ["ciao", "mi", "chiamo", "Dalila"];

const lengthString = (array) => {
  const newArray = arrayString.map((string) => string.length);
  return newArray;
};

console.log(lengthString(arrayString));
